...
export default function Modal({ setShowModal, user_id, token, tokenSecret }: ModalProps) {
     ...
     const handleCreateGroup = async () => {
          if (!groupType) {
               setErrorMessage('Please select a group type');
               setTimeout(() => {
                    setErrorMessage('');
               }, 3000);
               return;
                         ...
                         <div className="flex flex-row justify-between gap-[25px] mt-4">
                              {searchResults.length === 0 &&
                                   ...
                                   <>
                                        ...
                                        <button onClick={handleCreateGroup} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mb-4" type="button">
                                             Create
                                        </button>
                                   </>
                              }
